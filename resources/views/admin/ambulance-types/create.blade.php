                ...
                <div class="row mb-5">
                    <div class="col-12">
                        ...
                        <div class="alert alert-warning" role="alert">
                            <small><i class="bx bx-info-circle me-1"></i> Isi "Tarif Minimum" dan "Tarif/KM" di atas untuk membuat tier pertama secara otomatis</small>
                        </div>
                        <div id="tarifContainer">
                            <!-- Tier pertama otomatis diisi oleh JavaScript -->
                        </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            function updateFormState() {
                ...
                const hasEmptyRequiredInputs = $('input[required]').filter(function() {
                    return $(this).val() === '' || $(this).val() == 0;
                }).length > 0;
                
                const isFormInvalid = hasInvalidInputs || hasEmptyRequiredInputs;
                
                // Disable/enable submit button
                $('button[type="submit"]').prop('disabled', isFormInvalid);
                ...
                if (hasInvalidInputs) {
                    // Disable vehicle select and tarif inputs
                    $('#vehicle_id, #tarif_minimum, #tarif_per_km').prop('disabled', true);
                    ...
                    // Re-enable inputs
                    $('#vehicle_id, #tarif_minimum, #tarif_per_km').prop('disabled', false);
            ...
            // Validate tarif minimum and per km inputs
            $('#tarif_minimum, #tarif_per_km, #vehicle_id').on('input change', function() {
                updateFormState();
            });
            ...
            $('#form').on('submit', function(e) {
                ...
                // Validate tarif minimum and per km
                const tarifMinimum = parseFloat($('#tarif_minimum').val()) || 0;
                const tarifPerKm = parseFloat($('#tarif_per_km').val()) || 0;

                if (tarifMinimum <= 0) {
                    errorMessages.push('Tarif minimum harus lebih dari 0');
                    isValid = false;
                }

                if (tarifPerKm <= 0) {
                    errorMessages.push('Tarif per KM harus lebih dari 0');
                    isValid = false;
                }
