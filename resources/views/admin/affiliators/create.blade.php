...
<div class="container-xxl flex-grow-1 container-p-y">
    ...
    <div class="card mb-4">
        <div class="card-body">
            <form action="{{ route('admin.affiliators.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="row">
                    <div class="col-md-6">
                        <div class="mb-3">
                            <label class="form-label" for="name">Name</label>
                            <input type="text" class="form-control" id="name" name="name" value="{{ old('name') }}" placeholder="Enter affiliator full name" required />
                        </div>
                        <div class="mb-3">
                            <label class="form-label" for="code">Code</label>
                            <input type="text" class="form-control" id="code" name="code" value="{{ old('code') }}" placeholder="Enter affiliator code" required />
                        </div>

                        <div class="mb-3">
                            <label class="form-label" for="email">Email</label>
                            <input type="email" class="form-control" id="email" name="email" value="{{ old('email') }}" placeholder="Enter affiliator email" required />
                        </div>

                        <div class="mb-3">
                            <label class="form-label" for="phone_number">Phone</label>
                            <input type="text" class="form-control" id="phone_number" name="phone_number" value="{{ old('phone_number') }}" placeholder="Enter affiliator phone number" required />
                        </div>

                        <div class="mb-3">
                            <label class="form-label" for="full_address">Base Address</label>
                            <textarea class="form-control" id="full_address" rows="5" name="full_address" placeholder="Enter the base address of the affiliator" required>{{ old('full_address') }}</textarea>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="mb-3">
                ...
                <div class="mt-4">
                    <button type="submit" class="btn btn-primary">Save Affiliator</button>
